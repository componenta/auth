...
final readonly class DatabaseRefreshTokenStoreConfigFactory
{
    public function __invoke(
        ...
        return new DatabaseRefreshTokenStoreConfig(
            tokenTable: $config->string(new ConfigPath($prefix . '.tokenTable'), 'refresh_tokens'),
            familyTable: $config->string(new ConfigPath($prefix . '.familyTable'), 'refresh_token_families'),
            tokenHashColumn: $config->string(new ConfigPath($prefix . '.columns.tokenHash'), 'token_hash'),
            familyIdColumn: $config->string(new ConfigPath($prefix . '.columns.familyId'), 'family_id'),
