...
- UUIDs, hexadecimal hashes and refresh family IDs use byte-exact `ascii_bin`
  comparison;
- remember-me selectors are unique and validators are stored only as SHA-256 verifiers; one-time bearer representations are unique;
...
For remember-me grants, replace the old token-hash representation with a
32-character stable selector in the token column and add a non-null
64-character SHA-256 verifier column. Existing bearers cannot be migrated;
revoke existing remember-me grants during deployment.
