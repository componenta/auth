...
final class PasswordStrategyTest extends TestCase
{
    ...
    public function testUnknownIdentityIsDenied(): void
    {
        $strategy = new PasswordStrategy(
            new PasswordProviderFixture(null),
            new PasswordHasherFixture(),
            'dummy',
        );

        $result = $strategy->attempt(
            new Payload('missing@example.com', 'secret'),
            new Context(),
        );
    ...
    {
        $strategy = new PasswordStrategy(
            new PasswordProviderFixture(new PasswordIdentityFixture()),
            new PasswordHasherFixture(),
            'dummy',
        );

        $result = $strategy->attempt(
            new Payload('user@example.com', 'wrong-password'),
            new Context(),
        );
...
final class PasswordHasherFixture implements PasswordHasherInterface, PasswordVerifierInterface
{
    ...
    {
        return $password === 'secret' && $hash === 'hash';
    }
}
