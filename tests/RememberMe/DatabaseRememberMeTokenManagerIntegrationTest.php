...
{
    public function testReusingSupersededBearerRevokesCompromisedGrant(): void
    {
        self::requireSqlite();
        ...
        $replay = $manager->rotate($plainToken);
        self::assertInstanceOf(RememberMeCompromise::class, $replay);
        self::assertSame(['old-session'], $replay->sessionIds);
        self::assertSame(
            0,
            $database->select()->from('remember_me_tokens')->count(),
        );
        self::assertNull($manager->rotate($rotation->successorToken));
    }

    public function testRevokingOriginalBearerAfterRotationRevokesSuccessor(): void
    {
        self::requireSqlite();
        $database = SqliteDatabaseFixture::create();
        self::createSchema($database);
